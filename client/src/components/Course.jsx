...
const Course = ({list = "Grid", course = {}}) => {
    ...
    return (
        ...
        <Space direction={layout} className={`group rounded-md overflow-hidden courses-card-body ${list === "Grid" ? 'w-[290px] ' : 'w-full '} cursor-pointer`} onClick={() => navigate('/courses/' + course._id)} >
            <div className="w-full h-[164px] relative overflow-hidden">
                <img src={course.thumbnail ? "https://demo.creativeitem.com/academy/uploads/thumbnails/course_thumbnails/optimized/course_thumbnail_default-new_211689234810.jpg" : course.thumbnail } alt="" className="w-full h-full hover:scale-105 duration-200" />
            ...
            <div className="px-3 py-4">
                <h5 className="group-hover:text-[#754FFE] mb-2 font-semibold text-base">{course.title}</h5>
                ...
                <Flex align="center" justify="space-between" className="pt-4 border-t-[0.5px] border-[#6e798a]">
                    <Flex align="center" gap={8} className="text-base">
                        <h4 className="font-bold text-[#1E293B] group-hover:text-[#754FFE]">{!course.price ? "Free" : course.price.toLocaleString() + " VND"}</h4>
