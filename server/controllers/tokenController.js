    ...
    if (token && req.headers.authorization.startsWith('Bearer')) {
        try {
            ...
            if (error.name === 'TokenExpiredError') {
                ...
                try {
                    const decodedRefreshToken = jwt.verify(refreshToken, JWT_SECRET_REFRESH_TOKEN);
                    const user = await userModel.get({ id: decodedRefreshToken._id });
                    const newAccessToken = jwt.sign({ _id: user.id }, JWT_SECRET_ACCESS_TOKEN, { expiresIn: '15m' });
